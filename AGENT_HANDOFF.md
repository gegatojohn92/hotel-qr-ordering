...
- **Guest Chat FCM Push Notification Link & Alarm Execution (2026-09-08)**:
  ...
  - **Root Cause Analysis**:
    1. `apps/web/lib/webPush.ts`: When `notification_settings` exists in the database, the server applied custom role filtering against `notifSettings.frontdesk_allowed_types`. In the database, `frontdesk_allowed_types` contained `['CALL_REQUEST', 'TASK', 'FOOD_ORDER', 'SPA_BOOKING']` — it did NOT contain `'GUEST_CHAT'` or `'CHAT_HANDOFF'`. As a result, all staff members with the `FRONT_DESK` role (including active device `admin@gmail.com` with `ExponentPushToken[XwoAAtHDDvB_Tlk3CLV1d7]`) were filtered out, resulting in 0 Expo push messages dispatched.
    2. `apps/web/app/admin/settings/page.tsx`: The Admin Settings UI lacked checkboxes for `Guest Chat` and `Chat Handoff` in the Front Desk matrix, causing saved settings to exclude chat categories.
    3. `apps/web/app/api/chat/send/route.ts`: During `BOT_ACTIVE` mode, push notifications were previously gated behind `if (aiResponse.shouldEscalate)`. Regular guest messages never triggered any push notification to staff.
    4. `apps/staff-app/lib/notifications.ts`: The client-side `canRoleReceiveNotification` filter for `FRONT_DESK` was missing `CHAT_HANDOFF` and `GUEST_CHAT`, causing local notification filtering to silently drop chat alerts.
    5. `apps/staff-app/App.tsx`: The push response handler caught `CHAT_HANDOFF` and `GUEST_CHAT` and called `setRefreshKey` followed by an immediate `return`, failing to invoke `triggerAlarmNotification()`.
  - **Resolution**:
    - **Database Update**: Updated `notification_settings.frontdesk_allowed_types` in Supabase to include `'CHAT_HANDOFF'` and `'GUEST_CHAT'`.
    - `apps/web/lib/webPush.ts`: Added hard-coded pass-through for `CHAT_HANDOFF` and `GUEST_CHAT` in `FRONT_DESK`, `HOUSEKEEPING`, and `MAINTENANCE` role checks even when custom settings exist. Added `title`, `body`, and `conversationId` into `expoMessages.data` payload.
    - `apps/web/app/admin/settings/page.tsx`: Added `GUEST_CHAT` ("Guest Chat") and `CHAT_HANDOFF` ("Handoff") checkboxes under Front Desk routing matrix.
    - `apps/web/app/api/chat/send/route.ts` & `handoff/route.ts`: Explicitly pass `conversationId` to push payloads.
    - `apps/web/app/app/stay/components/GuestChatWidget.tsx`: Added `sessionStorage` fallback for `roomId` and `hotel_guest_last_room_id` so room identity persists during route changes.
    - `apps/staff-app/lib/notifications.ts`: Added `CHAT_HANDOFF` and `GUEST_CHAT` to `FRONT_DESK` role permissions in both default and custom settings branches.
    - `apps/staff-app/App.tsx`: Added `triggerAlarmNotification()` on push response for chat events, ensuring staff get audible/vibrating heads-up alerts. Added `setRefreshKey` in `addNotificationReceivedListener` for foreground updates.
